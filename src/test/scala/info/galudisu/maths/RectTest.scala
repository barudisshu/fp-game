...
class RectTest extends PropSpec with GeneratorDrivenPropertyChecks with Matchers {
  val genDouble: Gen[Double] = Gen.chooseNum(-999999.0, 999999.0, -1.0, 0.0, 1.0)
  ...
  val genEmptyRect: Gen[Rect] = for {
    x <- genDouble
    y <- genDouble
  } yield Rect(x, y, x, y)

  implicit val arbRect: Arbitrary[Rect] = Arbitrary(genRect)
  ...
  private def modifyRect(r: Rect)(x1f: BinOp, y1f: BinOp, x2f: BinOp, y2f: BinOp): Rect = {
    ...
    Rect(x1f(x1, hw), y1f(y1, hh), x2f(x2, hw), y2f(y2, hh))
  }
}
